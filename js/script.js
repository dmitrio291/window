document.addEventListener('DOMContentLoaded', () => {
    ...
    if (quizStep) {
        jQuery(document).ready(function($){ 
            ...
            //     $('.quiz-wrap').scrollTop( 0 );
            // }

            function scroll_to_quiz_wrap() {
                const quizWrapJs = $('.quiz-wrap-js').length;
                if (quizWrapJs) {
                    const scrollTop = $('.quiz-wrap-js').offset().top;
            ...
            let form = $('#quiz_form'), img_to_set_1 = $('.quiz-step-1-big-img','.quiz-showroom'),main=$('.quiz-container'),bprev=$('.quiz-nav-prev', main), bnext=$('.quiz-nav-next', main), currstep = 1, step= '.quiz-step-', stepmark = $('.quiz-nav-step'), pb=$('.quiz-progress-bar',form),pbl = $('.quiz-progress-label',form), nav = $('.quiz-nav',form),start_btn,ovrl=$('.quiz-overlay'),i_check;
        
            start_btn = 'button.quiz-button';
